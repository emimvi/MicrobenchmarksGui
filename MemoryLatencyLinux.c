...
uint32_t* preallocate_arr(uint32_t size_kb) {
    ...
    size_t testSizeKb = size_kb;
    size_t maxMemRequired = testSizeKb * (size_t)1024;
    maxMemRequired = (((maxMemRequired - 1) / hugePageSize) + 1) * hugePageSize;
    return mmap(NULL, maxMemRequired, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
}

int free_preallocate_arr(uint32_t* preallocate_arr, uint32_t size_kb) {
    size_t hugePageSize = 1 << 21;
    size_t testSizeKb = size_kb;
    size_t maxMemRequired = testSizeKb * (size_t)1024;
    maxMemRequired = (((maxMemRequired - 1) / hugePageSize) + 1) * hugePageSize;
    return munmap(preallocate_arr, maxMemRequired);
}
...
float RunLatencyTest(uint32_t size_kb, uint32_t iterations) {
    return RunTest(size_kb, iterations, mem);
}
float RunAsmLatencyTest(uint32_t size_kb, uint32_t iterations) {
    return RunAsmTest(size_kb, iterations, mem);
}

int SetLargePages(uint32_t enabled) {
    const uint32_t oneGBinKB = 1048576; //TODO FIXME hardcoded 1GB
    if (enabled && !mem) {
        mem = preallocate_arr(oneGBinKB);
        if (mem == (void *)-1) { // on failure, mmap will return MAP_FAILED, or (void *)-1
            mem = NULL;
            return -1;
        }
        return 0;
    } else if (!enabled && mem) {
        return free_preallocate_arr(mem, oneGBinKB);
    }
}
